...
public class BuildEditPresenterTest {
    ...
    public void testOnItemClick() throws Exception {
        ...
        mPresenter.onItemClick(mockFetcher, mView.getBinding().editDialogCurrentTitle.getText().toString(), "3.14.0");
        assertEquals("버전 하위 단위가 하나일땐 그 다음 요소를 선택해야한다 ", "3.14.0-alpha-1/", activity.getBinding().mainVersionName.getText().toString());

        mPresenter.onItemClick(mockFetcher, mView.getBinding().editDialogCurrentTitle.getText().toString(), "3.18");
