...
public interface BuildEditContract {
    enum FLAG {
        EDIT,   //EDIT - 일반적인 QA 빌드 를 나타내는 FLAG (BuildEditDialog 에서 가장 일반적으로 사용 되는 FLAG)
        APK,    //APK - MainPresenter.selectMyAbiBuild 메소드 호출시 abi에 맞는 apk가 없을때 (오래 된 빌드) 사용 되는 FLAG
        PR, //PR - PR 빌드 를 다운로드 하기 위한 FLAG
        SELECTOR,   //SELECTOR - BuildEditDialog 를 show 했을때 EDIT, MASTER, PR 을 선택할때 사용되는 FLAG
        MASTER, //MASTER - Master Branch 빌드를 다운로드 하기 위한 FLAG
        MASTER_APK  //MASTER_APK - Master Branch 에서 빌드 버젼 선택후 APK(qatest, release) 를 선택할때 사용되는 FLAG
    }
